...
| V5 | Validation, sanitization, encoding | partial | JSON Schema on ALL extractor ingest (all three extraction modes now: Claude Code operator, deterministic per-provider rules, and local-LLM/Ollama — SEC-025); the in-flow deterministic extractor's output is asserted schema-valid against the shared v1 contract in tests (tests/test_worker_autoextract.py, SEC-018) and routed to staging, never the catalog; local-LLM output gets an added client-side reject-and-log pass (worker/local_extract.py) before it ever reaches the same server-side pydantic validation the other two modes rely on; parameterized queries only; output encoding via framework templates (grep-verified: no \|safe/mark_safe/autoescape-off); HTTP request surface swept (SEC-022): open-redirect guard on `next`, RFC 5545 ICS escaping, bounded worker `?n`, URLField scheme validation blocks javascript:/data: links | SEC-005, SEC-018, SEC-022, SEC-025 | 2026-07-21 |
...
| V8 | Data protection | partial | No evidence docs (SEC-001); Gmail scan implemented stronger than extract-and-discard — metadata-only fetch (bodies never leave Google), per-scan online token confined to one request frame, signed user-bound state (tests/test_gmail_execution.py, SEC-020); full account deletion; data classification table in spec §7; local-to-prod catalog bootstrap load strips cross-environment user attribution before transfer (SEC-026) | SEC-001, SEC-003, SEC-020, SEC-026 | 2026-07-22 |
...
| V11 | Business logic | partial | Crawl-on-demand gate for community sources (SEC-007) verified in code + tests (offer/badge/source submissions all inert; spend triggers approver-only per SEC-013 audit); waitlist cap logic; lease TTL on job claims. **2026-07-21**: found an ad hoc local-session script that automated the Approver-click step for `SourceSubmission` promotion without checking `origin` — no production code affected (`SourceSubmissionAdmin.trigger_crawl` unchanged), but flagging since it's adjacent to this control; remediated + recorded as SEC-024 (session-scoped, single-operator local instance only; explicit real-time Approver authorization substitutes for the per-row admin click) | SEC-007, SEC-013, SEC-024 | 2026-07-21 |
